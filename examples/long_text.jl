...
lines = 0
test_type = Cint(0)
log = CImGui.TextBuffer()
"""
    show_app_long_text(p_open::Ref{Bool})
Test rendering huge amount of text, and the incidence of clipping.
"""
global function show_app_long_text(p_open::Ref{Bool})
    ...
    if CImGui.Button("Add 1000 lines")
        ...
        foreach(i->CImGui.Appendf(log, "$(lines+i) The quick brown fox jumps over the lazy dog\n"), 1:1000)
        lines += 1000
    end
