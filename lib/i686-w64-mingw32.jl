...
function Base.getproperty(x::Ptr{ImGuiDockNode}, f::Symbol)
    ...
    f === :IsVisible && return Ptr{Bool}(x + 148)
    f === :IsFocused && return (Ptr{Bool}(x + 148), 1, 1)
    f === :HasCloseButton && return (Ptr{Bool}(x + 148), 2, 1)
    f === :HasWindowMenuButton && return (Ptr{Bool}(x + 148), 3, 1)
    f === :WantCloseAll && return (Ptr{Bool}(x + 148), 4, 1)
    f === :WantLockSizeOnce && return (Ptr{Bool}(x + 148), 5, 1)
    f === :WantMouseMove && return (Ptr{Bool}(x + 148), 6, 1)
    f === :WantHiddenTabBarUpdate && return (Ptr{Bool}(x + 148), 7, 1)
    f === :WantHiddenTabBarToggle && return Ptr{Bool}(x + 149)
    f === :MarkedForPosSizeWrite && return (Ptr{Bool}(x + 149), 1, 1)
    return getfield(x, f)
end
