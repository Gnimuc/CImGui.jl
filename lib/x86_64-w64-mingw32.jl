...
function _getptr(x::Ptr{tm}, f::Symbol)
...
function Base.getproperty(x::Ptr{tm}, f::Symbol)
    f === :tm_sec && return unsafe_load(_getptr(x, f))
    f === :tm_min && return unsafe_load(_getptr(x, f))
    f === :tm_hour && return unsafe_load(_getptr(x, f))
    f === :tm_mday && return unsafe_load(_getptr(x, f))
    f === :tm_mon && return unsafe_load(_getptr(x, f))
    f === :tm_year && return unsafe_load(_getptr(x, f))
    f === :tm_wday && return unsafe_load(_getptr(x, f))
    f === :tm_yday && return unsafe_load(_getptr(x, f))
    f === :tm_isdst && return unsafe_load(_getptr(x, f))
    return getfield(x, f)
end
